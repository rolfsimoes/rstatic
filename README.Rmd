...
information assets. The specification can be consulted at
[https://stacspec.org/](https://stacspec.org/).

`rstatic` provides small, composable **primitives** to build and serialize
static STAC documents (catalogs, collections, items, assets, and links). It is
designed to be a reusable foundation that other packages (e.g. STAC
generators, experiment catalog builders) can rely on. It implements the STAC
specification version 1.0.0.

Optional features rely on suggested packages and degrade gracefully when they
are not installed:

- **`terra`** &mdash; bounding-box extraction (`extract_bbox()`) and thumbnail
  rendering (`new_thumbnail()`).
- **`xml2`** &mdash; QGIS style parsing (`qml_to_style()`).

If these packages are unavailable, you can still build documents by supplying
bounding boxes, geometries, and styles manually.
